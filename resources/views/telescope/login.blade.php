...
<head>
    ...
    <title>{{ __('Telescope Login') }}</title>
...
<body class="min-h-screen flex items-center justify-center p-6">
    <div class="max-w-md w-full bg-white rounded-lg shadow-xl p-6">
        <h1 class="text-xl font-semibold text-gray-900">{{ __('Telescope Login') }}</h1>
        <p class="text-sm text-gray-600 mt-2">{{ __('Enter your mobile number to receive a one-time code.') }}</p>
        ...
        <form method="POST" action="{{ route('telescope.send-code') }}" class="mt-6">
            @csrf
            <label for="mobile" class="block text-sm font-medium text-gray-900 mb-2">{{ __('Mobile number') }}</label>
            <input id="mobile" type="text" name="mobile" value="{{ old('mobile') }}" required autofocus
                   inputmode="numeric" pattern="[0-9]*" placeholder="09123456789"
                   class="@error('mobile') border-red-500 @enderror">
            @error('mobile')
                <p class="text-red-600">{{ $message }}</p>
            @enderror
            <button type="submit" class="mt-4">{{ __('Send code') }}</button>
        </form>
    </div>
</body>
</html>
