...
class PumpIrrigationReportRequest extends FormRequest
{
    ...
    {
        return [
            'start_date' => ['required', 'string', function ($attribute, $value, $fail) {
                if (!is_jalali_date($value)) {
                    $fail('The ' . $attribute . ' must be a valid Jalali date in Y/m/d format.');
                }
            }],
            'end_date' => ['required', 'string', function ($attribute, $value, $fail) {
                if (!is_jalali_date($value)) {
                    $fail('The ' . $attribute . ' must be a valid Jalali date in Y/m/d format.');
                }
            }],
