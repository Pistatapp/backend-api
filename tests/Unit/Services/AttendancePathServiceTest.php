<?php

namespace Tests\Unit\Services;

use App\Models\AttendanceGpsData;
use App\Models\User;
use App\Services\AttendancePathService;
use Carbon\Carbon;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

class AttendancePathServiceTest extends TestCase
{
    use RefreshDatabase;

    private AttendancePathService $service;

    protected function setUp(): void
    {
        parent::setUp();
        $this->service = new AttendancePathService();
    }

    /**
     * Test get user path returns GPS data for specific date.
     */
    public function test_get_user_path_returns_gps_data_for_specific_date(): void
    {
        $user = User::factory()->create();
        $date = Carbon::parse('2024-11-15');

        $gpsData1 = AttendanceGpsData::factory()->create([
            'user_id' => $user->id,
            'date_time' => $date->copy()->setTime(8, 0, 0),
            'coordinate' => ['lat' => 35.6892, 'lng' => 51.3890, 'altitude' => 1200],
        ]);

        $gpsData2 = AttendanceGpsData::factory()->create([
            'user_id' => $user->id,
            'date_time' => $date->copy()->setTime(12, 0, 0),
            'coordinate' => ['lat' => 35.6895, 'lng' => 51.3895, 'altitude' => 1200],
        ]);

        AttendanceGpsData::factory()->create([
            'user_id' => $user->id,
            'date_time' => $date->copy()->addDay()->setTime(8, 0, 0),
            'coordinate' => ['lat' => 35.6892, 'lng' => 51.3890, 'altitude' => 1200],
        ]);

        $path = $this->service->getUserPath($user, $date);

        $this->assertCount(2, $path);
        $this->assertEquals($gpsData1->id, $path->first()['id']);
        $this->assertEquals($gpsData2->id, $path->last()['id']);
    }

    /**
     * Test get user path returns empty collection when no GPS data.
     */
    public function test_get_user_path_returns_empty_collection_when_no_gps_data(): void
    {
        $user = User::factory()->create();
        $date = Carbon::parse('2024-11-15');

        $path = $this->service->getUserPath($user, $date);

        $this->assertCount(0, $path);
    }

    /**
     * Test get user path orders points by time.
     */
    public function test_get_user_path_orders_points_by_time(): void
    {
        $user = User::factory()->create();
        $date = Carbon::parse('2024-11-15');

        $gpsData3 = AttendanceGpsData::factory()->create([
            'user_id' => $user->id,
            'date_time' => $date->copy()->setTime(16, 0, 0),
            'coordinate' => ['lat' => 35.6900, 'lng' => 51.3900, 'altitude' => 1200],
        ]);

        $gpsData1 = AttendanceGpsData::factory()->create([
            'user_id' => $user->id,
            'date_time' => $date->copy()->setTime(8, 0, 0),
            'coordinate' => ['lat' => 35.6892, 'lng' => 51.3890, 'altitude' => 1200],
        ]);

        $gpsData2 = AttendanceGpsData::factory()->create([
            'user_id' => $user->id,
            'date_time' => $date->copy()->setTime(12, 0, 0),
            'coordinate' => ['lat' => 35.6895, 'lng' => 51.3895, 'altitude' => 1200],
        ]);

        $path = $this->service->getUserPath($user, $date);

        $this->assertEquals($gpsData1->id, $path->first()['id']);
        $this->assertEquals($gpsData2->id, $path->get(1)['id']);
        $this->assertEquals($gpsData3->id, $path->last()['id']);
    }

    /**
     * Test get user path formats data correctly.
     */
    public function test_get_user_path_formats_data_correctly(): void
    {
        $user = User::factory()->create();
        $date = Carbon::parse('2024-11-15');

        $gpsData = AttendanceGpsData::factory()->create([
            'user_id' => $user->id,
            'date_time' => $date->copy()->setTime(8, 0, 0),
            'coordinate' => ['lat' => 35.6892, 'lng' => 51.3890, 'altitude' => 1200],
            'speed' => 5.5,
            'bearing' => 90.25,
            'accuracy' => 10.75,
            'provider' => 'gps',
        ]);

        $path = $this->service->getUserPath($user, $date);
        $point = $path->first();

        $this->assertArrayHasKey('id', $point);
        $this->assertArrayHasKey('coordinate', $point);
        $this->assertArrayHasKey('speed', $point);
        $this->assertArrayHasKey('bearing', $point);
        $this->assertArrayHasKey('accuracy', $point);
        $this->assertArrayHasKey('provider', $point);
        $this->assertArrayHasKey('date_time', $point);
        $this->assertEquals($gpsData->id, $point['id']);
        $this->assertEquals('gps', $point['provider']);
    }

    /**
     * Test get latest point returns most recent GPS data.
     */
    public function test_get_latest_point_returns_most_recent_gps_data(): void
    {
        $user = User::factory()->create();

        $oldGpsData = AttendanceGpsData::factory()->create([
            'user_id' => $user->id,
            'date_time' => Carbon::now()->subHours(2),
            'coordinate' => ['lat' => 35.6892, 'lng' => 51.3890, 'altitude' => 1200],
        ]);

        $latestGpsData = AttendanceGpsData::factory()->create([
            'user_id' => $user->id,
            'date_time' => Carbon::now()->subMinutes(5),
            'coordinate' => ['lat' => 35.6895, 'lng' => 51.3895, 'altitude' => 1200],
        ]);

        $latestPoint = $this->service->getLatestPoint($user);

        $this->assertNotNull($latestPoint);
        $this->assertEquals($latestGpsData->id, $latestPoint['id']);
        $this->assertNotEquals($oldGpsData->id, $latestPoint['id']);
    }

    /**
     * Test get latest point returns null when no GPS data.
     */
    public function test_get_latest_point_returns_null_when_no_gps_data(): void
    {
        $user = User::factory()->create();

        $latestPoint = $this->service->getLatestPoint($user);

        $this->assertNull($latestPoint);
    }
}
