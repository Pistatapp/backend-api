...
class TractorStoppageNotificationTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        $this->date = now();
        $this->stoppageDuration = 7200; // 2 hours in seconds
    ...
    {
        $notification = new TractorStoppageNotification(
            $this->tractor,
            $this->stoppageDuration,
            $this->threshold,
            $this->date
        );
        ...
        $this->assertEquals($this->stoppageDuration, $data['stoppage_duration']);
    ...
    public function it_sends_notification_through_database_and_firebase()
    {
        ...
        $this->user->notify(new TractorStoppageNotification(
            $this->tractor,
            $this->stoppageDuration,
            $this->threshold,
            $this->date
    ...
    public function it_formats_firebase_message_correctly()
    {
        ...
        $this->assertEquals('Tractor Stoppage Warning', $firebaseMessage->title);
        $this->assertStringContainsString($this->tractor->name, $firebaseMessage->body);
        $this->assertEquals($this->stoppageDuration, $firebaseMessage->data['stoppage_duration']);
