...
Resolve a scanned QR code payload (`unique_id`) to a farm entity and return the **same JSON shape** as that entity’s normal `show` endpoint (including eager-loaded relations). The value encoded in QR images generated by the app is the plain-text `unique_id` string (not the base64 image).
...
1. **Scanning:** Decode the QR to the **text** payload and send it as `unique_id`. Do not send the raw PNG/SVG or the base64 `qr_code` column unless it is known to decode to the same string.
